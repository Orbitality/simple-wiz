        ...
        if($dayStamp > $dateDiff && $wallet == $to_address){
            if($amount >= $tronPrice && $amount <= $tronPrice+1 ){
                ...
                if($payType == "BUY_SUB"){
                    ...
                    for($i =1; $i<= $accountCount; $i++){
                        ...
                        if(is_null($response)){
                            ...
                            $stmt->execute();
                            $stmt->close();
                            sendMessage("تراکنش شما با تکسید آیدی $hash_id تأیید شد، ولی اتصال به سرور برقرار نیست، لطفا به مدیریت اطلاع دهید\n مبلغ " . number_format($price). " تومان ($tronPrice ترون) به حساب شما اضافه شد",null,null,$user_id);
                ...
                exit;
                        ...
                        $stmt->execute();
                        $stmt->close();
                        sendMessage("تراکنش شما با تکسید آیدی $hash_id تأیید شد، ولی به دلیل مشکل فنی امکان افزایش حجم نیست، لطفا به مدیریت اطلاع بدهید\n مبلغ " . number_format($price). " تومان به حساب شما اضافه شد",null,null,$user_id);
